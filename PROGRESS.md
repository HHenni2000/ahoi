...
| Phase 2 | Backend API Setup (VPS) | ?? 97% - API l?uft, Cron fehlt |
| Phase 3 | Expo Frontend | ?? 90% - Backend-Integration & Map fertig |
...
- [x] Playwright-Support f?r JavaScript-Seiten (Kindaling etc.)
- [x] Mit echten URLs testen
...
- [x] String-Normalisierung f?r konsistentes Hashing

### 1.5 Lokales Testing
- [x] Test mit Quelle 1: B?cherhallen Hamburg (5 Events, regex navigation)
...
- [x] Test mit Quelle 4: Museum f?r Kunst und Gewerbe (2 Events)
...
- [x] Token-Kosten messen: **~15.000 Tokens, ca. $0.0043 f?r 7 Quellen**

### 1.6 Scraper-Verbesserungen (Feb 2026)
- [x] iFrame-Content wird eingebettet (inkl. Google Sheets CSV-Extraktion)
- [x] Navigator-Scoring: Spielplan/Termine bevorzugt, Tickets/Repertoire abgewertet
- [x] Detail-Links werden priorisiert und URLs normalisiert
- [x] Zeitraum-Events (date_end) werden im Query ?berlappend gefiltert
- [x] Geocoding via Nominatim + Cache
- [x] Debug-Logging f?r Navigator/Extractor
...
- [x] Pydantic Models f?r API
- [x] CRUD Endpoints f?r Events
- [x] CRUD Endpoints f?r Sources
- [x] Scraper-Integration

### 2.2 Scripts
- [x] scrape_all.py (f?r Cron)
...
- [x] API l?uft und ist von au?en erreichbar (http://72.60.80.95:8000)
- [x] Firewall konfiguriert (Port 8000 offen)
- [ ] Cron Jobs einrichten (scrape_all.py, cleanup.py)
- [x] Erste Event-Quelle hinzuf?gen und testen
- [ ] Optional: Nginx Reverse Proxy f?r HTTPS
...
(Wird nach Phase 3 ausgef?llt)

---

## Notizen & Erkenntnisse

### Architektur-?nderung (2026-02-04)
- **Urspr?nglich geplant:** Firebase (Firestore + Cloud Functions)
...
- **Navigation Discovery**: Regex funktioniert f?r die meisten Seiten, LLM-Fallback f?r komplexere Navigation (z.B. Single-Page-Apps, Anchor-Links)
...
- **Playwright**: Notwendig f?r JS-lastige Seiten (Kindaling), auch als Fallback bei SSL-Problemen
- **Problematische Seiten**: klick-kindermuseum.de hat defektes SSL (Server-Problem)

### iFrame-Quellen
- Spielpl?ne k?nnen in iFrames (z.B. Google Sheets) eingebettet sein
- iFrame-Inhalte werden eingebettet und ggf. als CSV/Tabelle extrahiert

### JavaScript-Seiten (Playwright)
Folgende Domains ben?tigen Playwright f?r JavaScript-Rendering:
...
- **Process Manager:** PM2 statt Systemd - einfacher f?r bestehende PM2-Infrastruktur
- **PM2 Python-Problem:** PM2 kann Python-Binaries nicht direkt starten ? L?sung: `start.py` Wrapper-Script
- **Port-Konflikt:** Alte Prozesse mit `fuser -k 8000/tcp` killen
- **Firewall:** UFW Port 8000 ?ffnen mit `ufw allow 8000/tcp`
- **API l?uft:** http://72.60.80.95:8000/api/health erfolgreich von au?en erreichbar
- **N?chste Schritte:** Cron Jobs f?r automatisches Scraping, HTTPS via Reverse Proxy
